...
- **Les réglages de performance passent par le SC2 comme en Wi-Fi direct** : `MaxRotationSpeed (1,11,1)`, `MaxVerticalSpeed (1,11,0)`, `MaxTilt (1,2,1)` sont des ARCommands `ardrone3`, donc relayées. Les bornes sont annoncées par le drone dans `SpeedSettingsState (1,12,0)/(1,12,1)` et `PilotingSettingsState (1,6,1)` — **3 floats (courant, min, max)** — repoussées par `AllSettings (0,2,0)`. Code partagé entre les deux voies : `PerfSetting` dans `arsdk/ArsdkTransport.kt`.
